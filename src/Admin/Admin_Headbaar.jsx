...
export default function Admin_Headbaar() {
  ...
  return (
    <header className="bg-black text-white py-4 px-8 flex justify-between items-center sticky top-0 left-0 z-10 shadow-md shadow-white">
      ...
      <div className={`md:flex md:space-x-4 md:w-auto w-full md:h-auto h-full md:py-0 ${isDropdownOpen ? "left-0" : "left-[-100%]"} md:px-0 py-4 px-9 duration-300 bg-black md:bg-transparent fixed md:relative top-0 right-0 overflow-y-auto md:overflow-visible`}>
        ...
        {/* Home Link */}
        <div className="relative items-center md:hidden py-2.5 px-4 mb-1 rounded transition duration-200 hover:bg-white/10 gap-2 group">
          <div className='flex gap-2 items-center'>
            <MdHome className='text-2xl text-white' />
            <Link to="/" className="group-hover:translate-x-2 duration-300 font-semibold text-lg text-white">
              Home
            </Link>
          </div>
        </div>
        ...
        {menu_links?.map((data, index) => (
          <div
            key={index}
            className={`relative items-center md:hidden py-1 px-4 mb-1 rounded transition duration-200 ${highlight === data.name ? "bg-white text-black" : "text-white"} hover:bg-white/10 gap-2 group`}
          >
            <div className='flex gap-2 items-center'>
            ...
            <div className='gap-2 group-hover:flex hidden mt-1 px-2 rounded-e-md'>
              {data?.subitems?.map((subitem, subIndex) => (
                <Link
                  key={subIndex}
                  to={subitem.url}
                  className='text-black bg-white px-2 rounded-md hover:text-white hover:bg-black border border-white flex items-center gap-1 transition-all duration-200'
                  onClick={() => page_jump_handler(data)}
                >
                  {subitem?.icon}
                  {subitem.name}
        ...
        <div className={`md:hidden ${admin ? "block" : "hidden"} py-4 md:mb-0 mb-4`}>
          <Link
            to=""
            className="text-black text-lg font-semibold bg-white py-2 px-4 rounded-md hover:text-white hover:bg-black border border-white transition-all duration-200"
            onClick={Adminlogout_handler}
          >
            Logout
          </Link>

          <button className="flex items-center gap-2 mt-5 text-white" onClick={() => setIsDropdownOpen(false)}>
        ...
        <i className="fa-solid fa-xmark absolute top-5 right-5 text-2xl md:hidden text-white" onClick={() => setIsDropdownOpen(false)}></i>
      </div>
      ...
      <div>
        <Link className="md:flex gap-2 hidden relative group capitalize items-center">
          <FaUserCircle className="text-3xl text-white" />
          <div className='text-lg font-semibold text-white'>{admin?.name}</div>
          <div
            className={`absolute group-hover:block py-6 ${admin ? "block" : "hidden"} duration-200 w-full text-center text-lg font-semibold hidden top-5`}
            onClick={Adminlogout_handler}
          >
            <button className='bg-black hover:bg-white text-white hover:text-black h-7 px-3 rounded-md border border-white transition-all duration-200'>
              Logout
            </button>
          </div>
        </Link>
