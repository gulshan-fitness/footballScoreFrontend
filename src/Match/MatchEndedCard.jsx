...
export default function MatchEndedCard() {
  ...
  const homeManager = particulerMatch?.lineups?.find(d => d.team?.id === particulerMatch?.teams?.home?.id)?.coach?.name ?? "NA";
  const awayManager = particulerMatch?.lineups?.find(d => d.team?.id === particulerMatch?.teams?.away?.id)?.coach?.name ?? "NA";
  const referee = particulerMatch?.fixture?.referee ?? "NA";
