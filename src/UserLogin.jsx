...
export default function UserLogin() {
  ...
  const handleSendOtp = () => {
    ...
    axios.post(`${import.meta.env.VITE_API_BASE_URL}${import.meta.env.VITE_OTP_URL}/otp_send`, { phone })
      .then((res) => {
        notify(res.data.msg, res.data.status);
        if (res.data.status === 1) setOtpSent(true);
      })
      .catch(() => notify("Error sending OTP", 0));
  };

  const handleVerifyOtp = () => {
    axios.post(`${import.meta.env.VITE_API_BASE_URL}${import.meta.env.VITE_USER_URL}loginWithOtp`, {
      phone: phone.replace("+", ""),
      otp
    })
      .then((res) => {
  ...
  return (
    <div className="px-3 w-full py-4 h-screen bg-[#000000]">
      <div className="w-full max-w-md p-8 mx-auto bg-[#000000] border border-[#ffffff] rounded-lg shadow-lg">
        <h2 className="text-2xl font-bold mb-6 text-center text-[#ffffff]">User Login</h2>
            ...
            <button
              ...
              className={`px-2 py-1 rounded border ${
                loginMethod === method
                  ...
                  : "bg-black border-[#ffffff] text-[#ffffff]"
              }`}
            >
              {label}
            </button>
        ...
        <Link
          to={"/usersignup"}
          className="mt-4 text-sm text-center justify-center items-center flex-wrap flex gap-1 text-[#ffffff]"
        >
